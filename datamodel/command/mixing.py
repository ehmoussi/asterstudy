...
from common import CachedValues
from ..general import CataMixing, no_new_attributes
...
def _type_check(astype, value):
    ...
    if isinstance(astype, (list, tuple)):
        for subtype in astype:
            try:
                return _type_check(subtype, value)
...
class KeysMixing(CataMixing, CompositeMixing, StorageMixing):
    ...
    def _rkeys(keywords, rkeys):
        for key, item in keywords.iteritems():
    ...
    def __setitem__(self, name, value):
        ...
        command = self._engine
        stage = self._engine.stage
        from .helper import register_cos
        register_cos(stage, command)
...
class Unit(Simple):
    ...
    def value(self, value):
        """Updates the value"""
        unregister_unit(self._engine, False)
